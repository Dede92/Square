...
class SquareTest(DjangoTestCase, SnapshotTestCase):
    ...
    def test_query_get_people_counter(self):
        ...
        executed = client.execute(query_get_people_counter, variables={'room': 'room_a'})
    ...
    def test_mutation_add_people_to_room(self):
        client = Client(schema)
        executed = client.execute(mutation_add_people_to_room, variables={'room': 'room_b', 'peopleId': 'Mac', 'statusIn': 0, 'statusOut': 1})
        self.assertMatchSnapshot(executed)
