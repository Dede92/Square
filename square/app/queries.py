...
class Query(graphene.ObjectType):
    ...
    get_people_counter = graphene.Field(PeopleCountType, room=graphene.String(required=True), atInstant=graphene.String())
    ...
    def resolve_get_people_counter(self, info, **kwargs):
        ...
        a = PeopleCounter.objects.filter(q).aggregate(num_in=Sum('status_in'), num_out=Sum('status_out'))
        return {
            'room': kwargs['room'],
